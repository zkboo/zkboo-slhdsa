...
//!
//! Key generation ([wots_pk_gen]) runs every chain to its full length with public control flow.
//! Signature verification ([wots_pk_from_sig]) starts each chain at a position determined by a
//! message digit, which is a secret wire in the circuit; every chain therefore performs all
//! `w − 1` hash steps, and a masked selection keeps the chain value unchanged until its secret
//! start position is reached, so the work done is independent of the message.
...
/// within each byte) followed by the 3 checksum digits (FIPS 205, Algorithm 8, steps 1–6).
...
/// Recomputes a WOTS+ public key from a signature and the signed `n`-byte value
/// (FIPS 205, Algorithm 8), with message-independent control flow.
///
