...
//!
//! SLH-DSA verification branches on values derived from the message digest (WOTS+ chain lengths,
//! Merkle authentication-path directions, tree indices), which are secret wires in the circuit.
//! Since circuit control flow must be data-independent, all such branching is replaced by
//! fixed-work computation combined with the masked selections implemented here.
...
/// Extracts bit `j` (counting from the least significant) of a u32 word, as a u8 word holding
/// 0 or 1.
...
///
/// Requires `1 <= b <= 25` (so that any `b`-bit group fits a 4-byte window) and enough input
/// bytes to cover `count * b` bits.
