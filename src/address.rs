...
impl<B: Backend> Adrs<B> {
    ...
    /// Returns the compressed 22-byte address `ADRSc` used by the SHA2 instantiations (FIPS 205,
    /// §11.2): one layer byte, eight tree-address bytes, one type byte, and the three
    /// type-dependent words.
