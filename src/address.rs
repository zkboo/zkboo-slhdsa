...
//!
//! An address is fed into every tweakable hash call. Its fields are big-endian integers at fixed
//! byte offsets: layer (bytes 0..4), tree address (4..16), type (16..20), and three type-dependent
//! words (20..24, 24..28, 28..32). During key generation all fields are public constants; during
//! verification the tree address, key-pair address, and tree index derive from the (secret)
//! message digest, so setters exist in both constant and wire flavours.
...
impl<B: Backend> Adrs<B> {
    ...
    /// Sets the type (bytes 16..20) and zeroes the three type-dependent words (bytes 20..32),
    /// as prescribed for every address-type change (FIPS 205, §4.2).
