...
/// Recomputes the SLH-DSA public-key root `PK.root` from the secret seed `SK.seed`
/// (FIPS 205, Algorithm 18, root computation).
///
/// `sk_seed` is the secret witness (typically circuit input wires); `pk_seed` is public.
/// The returned wires are meant to be circuit outputs, checked against the known public key.
...
/// Recomputes the SLH-DSA public-key root `PK.root` from a message and a signature
/// (FIPS 205, Algorithm 20), with signature-independent control flow.
///
/// `sig` is the secret witness (typically circuit input wires); `msg` is the *internal* message
/// `M'` (for the pure external interface with context `ctx`, that is `0x00 ‖ len(ctx) ‖ ctx ‖ M`).
/// The returned wires are meant to be circuit outputs: the signature is valid exactly if they
/// equal the known `PK.root`, which the proof verifier checks against the expected output.
